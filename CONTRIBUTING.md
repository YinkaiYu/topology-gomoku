...
完整方法见 [PR 审查与协作指南](docs/development/pr-review.md)。提交前至少回答：

- 除了功能两端，是否测试过中间状态、首次进入、异步回合与操作中断？展示变化是否保持棋局不变量？
- 是否查过稳定版本中的同类图标和控件，并比较了整套交互状态，而不只是静态外观？
- 隐藏控件会不会引起邻近按钮跳位？新控件是否抢占内容注意力？
- 行为测试是否执行生产逻辑？同状态证据、未测设备和已知限制是否写清楚？

收到 Request changes 后逐项回复修复提交与复验结果；认为意见不适用时，给出证据或说明替代方案，不只回复“已修复”。替代 PR 应关联原 PR 与评审，方便对照上一轮失败场景。维护者接手剩余修改时仍保留贡献署名，并对最终版本重新验证、预览和确认。
