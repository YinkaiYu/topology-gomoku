...
- 集成分支不是直接开发目录。共享任务从最新 `dev`、平台专属任务从对应发行分支新建独立的 `codex/<task>` 分支与 worktree；不要直接在 `dev`、`main` 或长期发行 worktree 中修改。
- 固定流程是：**独立分支/worktree → 实现与自动验证 → 本地预览 → 用户明确确认 → 再合并**。共享任务合回 `dev`，平台专属任务合回对应发行分支；预览或评审阶段不得提前合并。
...
- 视觉修改必须在任务 worktree 中用同一视口、同一内容和同一交互状态做前后对比，并把有效证据以仓库相对路径记录到 `design-qa.md`。本机临时路径、聊天附件路径和不可复现截图不能作为长期证据。
...
- 在 Pull Request 中说明目标、平台范围、实际验证、预览确认、视觉/真机检查和已知限制。
