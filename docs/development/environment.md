...
- `pv:platforms` 用于画面定版后的音频修订：保留 B 站 4K、抖音 9:16 与小红书 3:4 已确认的视频码流，只替换帧对齐最终混音并完整解码验证，结果写入 `.tmp/chapter-teaser/final-deliveries/`；该流程不生成 ProRes 4K 母版。
...
- 渲染器默认不覆盖现有文件；需要替换时显式传入 `--overwrite`。`pv:master` 仅在未来确有 10-bit 中间母版需求时使用，不是投稿成片的前置条件；确认前不合并任务分支。
