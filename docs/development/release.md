...
- 在核心 `dev` 中确认本轮唯一 SemVer，并检查包版本、字体/样式资源路径（不得带查询参数或片段）和发布说明一致。平台分支不能在此步骤之外决定新版本号。
...
| `package.json`、平台清单中的版本/缓存标识与游戏 SemVer | 接收本轮 `main` 的统一版本号，同时保留 Bilibili 专属命令和平台依赖 |
...
如果冲突暴露的是共享缺陷、共享契约不完整或多个渠道都需要的修复，按合并手册中止当前整合，从 `dev` 新建共享任务，验证并重新完成稳定提升后，再重做本次发行整合。无法确定文件职责时同样先暂停，查阅对应 adapter 和平台文档，而不是凭 `ours` / `theirs` 猜测。
...
- 平台分支中的通用修复必须回流 `dev`，不得复制到其他发行分支。
