...
2026-09-05 分支发行：从稳定 `main` 的共享运行时接收连续视角，保留 Toy SDK、沉浸模式、动态视口、键盘与平台字体边界。126 项测试、31 个包文件、25 份文档和 ZIP 构建通过。维护者明确批准先完成 Git 分支发行；本次未调用 Toy 上传或提交审核，真机与宿主预览仍待完成。下文 2026-09-01 数据是 1.38.0 的历史基线，不作为本轮实机验收证据。
