...
对局二维/三维视角切换（任务分支 `feature/board-view`）：在统一的 2216 × 1242 桌面预览视口下，第二关及更高关卡显示无外围面板的端点控制条，结构为“二维 — 连续滑块 — 三维”，终局阶段继续保留且可拖动回看。终局操作区在不重复显示视角按钮的前提下，以复盘、前后步、再来、下一关和旅程六项操作组成 3 × 2 布局。滑块直接驱动二维到三维的连续曲面形变；三维状态下按下超过 7 px 才进入旋转拖动，未超过阈值的释放才按按下瞬间的资格判断落子；二维仍保持原有落子交互。视角状态独立于棋盘、棋谱和当前回合，切换或滑块拖动期间棋盘落子入口与 AI 落子均锁定，稳定三维状态可继续落子。三维中产生胜利时从结束瞬间的真实形变进度与观察角度连续进入胜利连线展示；终局视角端点按钮在二维和三维之间复用可逆动画，动画期间锁定操作，完成后可继续拖动回看。教学和自动演示阶段控制条保留占位但隐藏交互内容，避免下方操作区跳动。终局曲面收回完成后同步清零对局视角，保持二维稳定态。任务 worktree 本地预览地址为 `http://127.0.0.1:4174/`；本轮代码修复待用户在同一预览确认后补充最终截图证据。
