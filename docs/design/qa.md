...
| 《足音回归》七流形汇聚与几何转场 | [`cylinder→torus 前`](../../artifacts/pv-transition-scenes-task7/task7-cylinder-to-torus-pre.png)、[`中`](../../artifacts/pv-transition-scenes-task7/task7-cylinder-to-torus-mid.png)、[`后`](../../artifacts/pv-transition-scenes-task7/task7-cylinder-to-torus-post.png)、[`torus→Möbius 中`](../../artifacts/pv-transition-scenes-task7/task7-torus-to-mobius-mid.png)、[`Möbius→Klein 中`](../../artifacts/pv-transition-scenes-task7/task7-mobius-to-klein-mid.png)、[`gallery 146.90s`](../../artifacts/pv-transition-scenes-task7/task7-gallery-withdrawal-146.90s.png)、[`gallery 148.40s / outro 重叠`](../../artifacts/pv-transition-scenes-task7/task7-gallery-withdrawal-148.40s.png) 与 [`关键帧联系表`](../../artifacts/pv-transition-scenes-task7-contact-sheet.png)；原生 3840 × 2160、60fps。运行时为每个 contract 建立互不覆盖的 occlusion / outgoing-match / incoming-match 三层，50.02s incoming 环形仍可见，148.40s gallery 在旁白期保持可辨且继续退镜。逐帧 contractId、phase、seek、expectedGeometry、几何 bbox 与像素方差见 [`manifest`](../../artifacts/pv-transition-scenes-task7-manifest.json)；证据门禁要求 `pixels.background` 恰含 red / green / blue 三通道并与 PNG 独立解码值吻合，`observation.geometry` 恰按 occlusion / outgoing-match / incoming-match 三层记录 geometry、opacity 与 bbox，并逐层复核 live runtime 及 expectedGeometry。联系表 SHA-256：`87a076df04c7bbc800206fa9dc83eb14cf1106c13e4ffb223ab495342191515a`。低码率 review render 因本机缺少 FFmpeg/FFprobe 未生成（Task 11 使用 doctor 发现路径处理），完整成片由 Task 11 输出 |
