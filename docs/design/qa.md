...
同日发行整合补充：共享运行时已通过 `4985e1c → b4beab2` 回流并提升，微信整合提交为 `1464f09`，共享 `app/` 与该稳定来源无差异。完整 `check:wechat` 通过 172 项测试、29 个 H5 包文件、25 份文档及 64 个微信构建文件；图标由最新 H5 按钮路径重新生成且内容未漂移。维护者明确批准仅完成所有分支发行，未完成的真机与自动化宿主项继续保留，不阻止本次 Git 同步；不据此声称已上传或已上线。
