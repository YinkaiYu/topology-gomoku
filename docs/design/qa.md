...
对局二维/三维视角切换（任务分支 `feature/board-view`）：在统一的 2216 × 1242 桌面预览视口下，第二关及更高关卡显示单一玻璃材质视角控制条，左侧按钮负责带动画的二维/三维切换，滑块两端标注“二维 / 三维”，并在终局阶段继续保留且可拖动回看。终局操作区在不重复显示视角按钮的前提下，以复盘、前后步、再来、下一关和旅程六项操作组成 3 × 2 布局。滑块直接驱动二维到三维的连续曲面形变；三维状态下按下超过 7 px 才进入旋转拖动，未超过阈值的释放才按投影交点落子；二维仍保持原有落子交互。视角状态独立于棋盘、棋谱和当前回合，切换动画期间棋盘落子入口锁定但允许拖动视角，稳定三维状态可继续落子。三维中产生胜利时跳过二维到曲面的终局形变，直接进入胜利连线展示；终局视角按钮在二维与三维之间均复用可逆的终局形变动画，动画期间锁定操作，完成后可继续拖动或对弈回看。终局曲面收回完成后同步清零对局视角，保持二维稳定态。任务 worktree 本地预览地址为 `http://127.0.0.1:4174/`；本次已在该预览完成终局布局、进入三维和返回二维动画交互确认，并将三种状态截图记录到 `artifacts/`。
