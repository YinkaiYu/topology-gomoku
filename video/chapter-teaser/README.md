...
该命令先以单字模块确定性重组 08D 与 09F 两套准确字标，再输出 `.tmp/chapter-teaser/cover-selection-exploration-v4/`。候选不再引用已排除的水墨笔触和二次元章节字，保留原球面基线作对照，并集中比较：放大黑白双生“五”、修正“棋”的几何字标、真实游戏 Logo、07B 几何流形、08 几何克莱因、4 × 4 格环游周期棋盘、七流形环绕、实体莫比乌斯、章节图鉴式莫比乌斯，以及图片生成的完整封面底图。所有方向均生成 4:3、16:9、3:4；竖版把横向底图作为可重排主视觉资产重新布局，不直接裁掉两侧对象。目录同时保存三个比例的总览、160 × 90 / 160 × 120 / 135 × 180 真实缩略图证明、字标候选板和哈希清单。
