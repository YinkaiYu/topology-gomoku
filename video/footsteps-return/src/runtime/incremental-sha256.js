const INITIAL_STATE = Object.freeze([
  0x6a09e667, 0xbb67ae85, 0x3c6ef372, 0xa54ff53a,
  0x510e527f, 0x9b05688c, 0x1f83d9ab, 0x5be0cd19
]);

const ROUND_CONSTANTS = Object.freeze([
  0x428a2f98, 0x71374491, 0xb5c0fbcf, 0xe9b5dba5, 0x3956c25b, 0x59f111f1, 0x923f82a4, 0xab1c5ed5,
  0xd807aa98, 0x12835b01, 0x243185be, 0x550c7dc3, 0x72be5d74, 0x80deb1fe, 0x9bdc06a7, 0xc19bf174,
  0xe49b69c1, 0xefbe4786, 0x0fc19dc6, 0x240ca1cc, 0x2de92c6f, 0x4a7484aa, 0x5cb0a9dc, 0x76f988da,
  0x983e5152, 0xa831c66d, 0xb00327c8, 0xbf597fc7, 0xc6e00bf3, 0xd5a79147, 0x06ca6351, 0x14292967,
  0x27b70a85, 0x2e1b2138, 0x4d2c6dfc, 0x53380d13, 0x650a7354, 0x766a0abb, 0x81c2c92e, 0x92722c85,
  0xa2bfe8a1, 0xa81a664b, 0xc24b8b70, 0xc76c51a3, 0xd192e819, 0xd6990624, 0xf40e3585, 0x106aa070,
  0x19a4c116, 0x1e376c08, 0x2748774c, 0x34b0bcb5, 0x391c0cb3, 0x4ed8aa4a, 0x5b9cca4f, 0x682e6ff3,
  0x748f82ee, 0x78a5636f, 0x84c87814, 0x8cc70208, 0x90befffa, 0xa4506ceb, 0xbef9a3f7, 0xc67178f2
]);

const rotateRight = (value, amount) => (value >>> amount) | (value << (32 - amount));

function bytesView(value) {
  if (value instanceof Uint8Array) return value;
  if (value instanceof ArrayBuffer) return new Uint8Array(value);
  if (ArrayBuffer.isView(value)) return new Uint8Array(value.buffer, value.byteOffset, value.byteLength);
  throw new TypeError("IncrementalSha256.update expects bytes");
}

export class IncrementalSha256 {
  constructor() {
    this.state = Uint32Array.from(INITIAL_STATE);
    this.block = new Uint8Array(64);
    this.blockLength = 0;
    this.bytesHashed = 0;
    this.finalized = false;
    this.digest = null;
  }

  update(value) {
    if (this.finalized) throw new Error("SHA-256 is already finalized");
    const bytes = bytesView(value);
    this.bytesHashed += bytes.byteLength;
    if (!Number.isSafeInteger(this.bytesHashed)) throw new Error("SHA-256 input is too large");
    let offset = 0;
    while (offset < bytes.byteLength) {
      const take = Math.min(64 - this.blockLength, bytes.byteLength - offset);
      this.block.set(bytes.subarray(offset, offset + take), this.blockLength);
      this.blockLength += take;
      offset += take;
      if (this.blockLength === 64) {
        this.transform(this.block);
        this.blockLength = 0;
      }
    }
    return this;
  }

  transform(block) {
    const words = new Uint32Array(64);
    for (let index = 0; index < 16; index += 1) {
      const offset = index * 4;
      words[index] = ((block[offset] << 24) | (block[offset + 1] << 16) | (block[offset + 2] << 8) | block[offset + 3]) >>> 0;
    }
    for (let index = 16; index < 64; index += 1) {
      const prior = words[index - 15];
      const recent = words[index - 2];
      const sigma0 = rotateRight(prior, 7) ^ rotateRight(prior, 18) ^ (prior >>> 3);
      const sigma1 = rotateRight(recent, 17) ^ rotateRight(recent, 19) ^ (recent >>> 10);
      words[index] = (words[index - 16] + sigma0 + words[index - 7] + sigma1) >>> 0;
    }

    let [a, b, c, d, e, f, g, h] = this.state;
    for (let index = 0; index < 64; index += 1) {
      const upper1 = rotateRight(e, 6) ^ rotateRight(e, 11) ^ rotateRight(e, 25);
      const choice = (e & f) ^ (~e & g);
      const temporary1 = (h + upper1 + choice + ROUND_CONSTANTS[index] + words[index]) >>> 0;
      const upper0 = rotateRight(a, 2) ^ rotateRight(a, 13) ^ rotateRight(a, 22);
      const majority = (a & b) ^ (a & c) ^ (b & c);
      const temporary2 = (upper0 + majority) >>> 0;
      h = g;
      g = f;
      f = e;
      e = (d + temporary1) >>> 0;
      d = c;
      c = b;
      b = a;
      a = (temporary1 + temporary2) >>> 0;
    }
    this.state[0] = (this.state[0] + a) >>> 0;
    this.state[1] = (this.state[1] + b) >>> 0;
    this.state[2] = (this.state[2] + c) >>> 0;
    this.state[3] = (this.state[3] + d) >>> 0;
    this.state[4] = (this.state[4] + e) >>> 0;
    this.state[5] = (this.state[5] + f) >>> 0;
    this.state[6] = (this.state[6] + g) >>> 0;
    this.state[7] = (this.state[7] + h) >>> 0;
  }

  digestBytes() {
    if (this.digest) return this.digest.slice();
    this.finalized = true;
    const tail = new Uint8Array(128);
    tail.set(this.block.subarray(0, this.blockLength));
    tail[this.blockLength] = 0x80;
    const paddedLength = this.blockLength < 56 ? 64 : 128;
    const high = Math.floor(this.bytesHashed / 0x2000_0000) >>> 0;
    const low = ((this.bytesHashed % 0x2000_0000) * 8) >>> 0;
    const lengthOffset = paddedLength - 8;
    tail[lengthOffset] = high >>> 24;
    tail[lengthOffset + 1] = high >>> 16;
    tail[lengthOffset + 2] = high >>> 8;
    tail[lengthOffset + 3] = high;
    tail[lengthOffset + 4] = low >>> 24;
    tail[lengthOffset + 5] = low >>> 16;
    tail[lengthOffset + 6] = low >>> 8;
    tail[lengthOffset + 7] = low;
    this.transform(tail.subarray(0, 64));
    if (paddedLength === 128) this.transform(tail.subarray(64));

    this.digest = new Uint8Array(32);
    for (let index = 0; index < this.state.length; index += 1) {
      const value = this.state[index];
      const offset = index * 4;
      this.digest[offset] = value >>> 24;
      this.digest[offset + 1] = value >>> 16;
      this.digest[offset + 2] = value >>> 8;
      this.digest[offset + 3] = value;
    }
    return this.digest.slice();
  }

  digestHex() {
    return [...this.digestBytes()].map((byte) => byte.toString(16).padStart(2, "0")).join("");
  }
}
