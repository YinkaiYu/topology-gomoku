...
- 最终草稿混音固定为 214.040 秒 / 48 kHz / stereo PCM-24。21 条 F `cold-witness` 干声按整数采样点等功率居中；完整重排的配乐保持 0.7 倍 M/S side level，并由旁白 sidechain 克制压低；21 个按当前语义画面事件推导的 SFX cue 仅覆盖 18.320 秒（全片 8.559%）且互不重叠，最后一个 `stone-outro` 从 201.484333333333 秒开始、持续 0.42 秒，因此当前 SFX 端点为 201.904333333333 秒，片尾不添加低频床。当前 FFmpeg 独立复测为 -14.06 LUFS integrated / -1.19 dBTP，母带精确包含 10,273,920 个采样帧；210.040 秒后的 D6/9 事件与 2.358 秒释放尾音保留，不以硬静音截断。
